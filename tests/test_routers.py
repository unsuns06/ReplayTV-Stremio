...
class TestStreamEndpoints:
    ...
    def test_proxied_stream_is_web_ready(self):
        """Self-contained proxy URLs must stay web-ready: no notWebReady/proxyHeaders,
        else Stremio re-proxies them and injects the wrong headers, breaking playback."""
        from app.routers.stream import _build_stream_from_info
        stream = _build_stream_from_info(
            {"url": "https://mf.example/proxy/hls/manifest.m3u8?d=x&api_password=p",
             "manifest_type": "hls", "title": "[HLS] TF1",
             "headers": {"authorization": "Bearer tok", "referer": "https://www.tf1.fr"}},
            include_ip_headers=False,
        )
        assert stream.behaviorHints is None


class TestErrorHandling:
    """Test error handling in endpoints"""

    @patch('app.providers.factory.ProviderFactory.create_provider')
...
class TestMagicNumbersReplaced:
    ...
    def test_cbc_uses_central_ttl_constants(self):
        """CBC uses CacheTTL from cache_keys instead of private TTL literals."""
        import pathlib
        src = pathlib.Path("app/providers/ca/cbc.py").read_text(encoding='utf-8')
        assert "CacheTTL" in src
        assert "ttl=3600" not in src
        assert "ttl=7200" not in src
        assert "ttl=1800" not in src

    def test_programs_loader_uses_central_ttl_constant(self):
        import pathlib
        src = pathlib.Path("app/utils/programs_loader.py").read_text(encoding='utf-8')
        assert "CacheTTL" in src
        assert "ttl=3600" not in src

    def test_routers_use_central_ttl_constants(self):
        """Catalog/meta routers must not re-declare their own TTL constants."""
        import pathlib
        for path in ("app/routers/catalog.py", "app/routers/meta.py"):
            src = pathlib.Path(path).read_text(encoding='utf-8')
            assert "CacheTTL" in src, f"{path} should use CacheTTL"
            assert "_CACHE_TTL =" not in src, f"{path} re-declares a TTL constant"
...
class TestPhase1Cleanup:
    ...
    def test_safe_print_removed(self):
        import pathlib
        assert not pathlib.Path("app/utils/safe_print.py").exists(), \
            "safe_print.py should be deleted — use logging module instead"
    ...
    def test_drm_mixin_check_processed_file_uses_session(self):
        """_check_processed_file lives on DRMProcessedFileMixin and uses self.session."""
        import pathlib
        src = pathlib.Path("app/providers/drm_mixin.py").read_text(encoding='utf-8')
        start = src.index("def _check_processed_file")
        end = src.index("\n    def ", start + 1)
        body = src[start:end]
        assert "import requests" not in body, "_check_processed_file must not use inline `import requests`"
        assert "self.session" in body, "_check_processed_file must use self.session for HTTP calls"
