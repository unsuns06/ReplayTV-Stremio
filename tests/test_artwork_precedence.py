...
def test_first_non_empty_candidate_wins(provider):
    picked = provider._pick_artwork({"logo": [None, "", "second.png", "third.png"]}, {})
    assert picked["logo"] == "second.png"


def test_pinned_field_is_skipped_entirely(provider):
    picked = provider._pick_artwork({"logo": ["api.png"], "poster": ["p.jpg"]},
                                    {"logo": "pinned.png"})
...
def test_field_with_no_candidates_is_omitted(provider):
    ...
    assert provider._pick_artwork({"logo": [None, ""]}, {}) == {}


def test_empty_pin_does_not_count_as_pinned(provider):
    assert provider._pick_artwork({"logo": ["api.png"]}, {"logo": ""}) == {"logo": "api.png"}


def test_enhance_series_meta_only_writes_string_urls(provider, monkeypatch):
    """FranceTV-shaped extras carry lists/dicts; those must not land in the meta."""
    provider.shows = {"show": {}}
    monkeypatch.setattr(provider, "_get_show_api_metadata",
                        lambda sid, info: {"logo": "l.png", "images": [1, 2], "description": None})
...
def test_self_fetching_providers_share_the_rule(key):
    """Each unified provider drives its artwork through _pick_artwork."""
