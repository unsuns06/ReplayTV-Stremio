#!/usr/bin/env python3
"""
CBC Provider Tests - Baseline
Run these BEFORE refactoring to establish working baseline.
Run again AFTER refactoring to verify no regressions.

Note: CBC live channels are marked as non-functional per user requirements.
Only testing replay functionality.
"""
...
def test_get_episodes(show_id: str = None):
    ...
    if not show_id:
        # Use Dragon's Den
        show_id = "cutam:ca:cbc:dragons-den"
...
def test_get_episode_stream_url(episode_id: str = None):
    ...
    try:
        ...
        if stream is None:
            log("⚠️ Stream returned None (may need authentication or geo-block)")
        ...
        if isinstance(stream, dict):
            ...
            log("✅ get_episode_stream_url: Got stream response")
...
def run_all_tests():
    """Run all CBC tests and save results"""
    log("=" * 60)
    log("🚀 CBC Provider Baseline Tests (Replay Only)")
    log("=" * 60)
    ...
    # Save results
    results_path = os.path.join(os.path.dirname(__file__), "cbc_baseline.json")
