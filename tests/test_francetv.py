#!/usr/bin/env python3
"""
FranceTV Provider Tests - Baseline
...
def test_get_episodes(show_id: str = None):
    ...
    if not show_id:
        # Use first show - "envoye-special"
        show_id = "cutam:fr:francetv:envoye-special"
...
def test_get_channel_stream_url(channel_id: str = None):
    ...
    if not channel_id:
        channel_id = "cutam:fr:francetv:france-2"
    ...
    try:
        ...
        log("✅ get_channel_stream_url: Got stream response")
...
def run_all_tests():
    """Run all francetv tests and save results"""
    log("=" * 60)
    log("🚀 FranceTV Provider Baseline Tests")
    log("=" * 60)
    ...
    # Save results
    results_path = os.path.join(os.path.dirname(__file__), "francetv_baseline.json")
