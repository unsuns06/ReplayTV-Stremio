...
class FranceTVImageExtractor:
    """Centralises FranceTV API image URL extraction.

    All three previous copies of "iterate patterns, match type, pick width,
    prepend https://www.france.tv" are replaced by calls to this class.
    """

    _BASE_URL = "https://www.france.tv"

    # Maps pattern type substring → preferred width keys tried in order
    _TYPE_WIDTH_PREFS: Dict[str, List[str]] = {
    ...
    def best_url(self, pattern: Dict[str, Any], type_key: str) -> str:
        """Return the best available URL from a single image pattern dict."""
        urls = pattern.get("urls", {})
        for width in self._TYPE_WIDTH_PREFS.get(type_key, ["w:400"]):
            url = urls.get(width)
            if url:
    ...
    def extract(
        ...
        Args:
            patterns: The ``media_image.patterns`` list from the API.
            wanted: ``{result_key: type_substring}`` — which types to look for
                and what to call them in the result.
                Example: ``{"poster": "vignette_3x4", "logo": "logo"}``.

        Returns:
            Dict of ``{result_key: absolute_url}`` for every type found.
        """
        result: Dict[str, str] = {}
        for pattern in patterns:
            ptype = pattern.get("type", "")
            for result_key, type_key in wanted.items():
                if result_key not in result and type_key in ptype:
                    url = self.best_url(pattern, type_key)
                    if url:
                        result[result_key] = url
...
image_extractor = FranceTVImageExtractor()
